...
def chk():
  ...
  try:
    ...
    if id in httpCaht:
      print("\033[92m👑♡  𝐬𝐨𝐱Ŧ  👑♡ LOGIN SECCESFULL")
      msg = str(os.geteuid())
      time.sleep(1)
      pass
    else:
      print("\x1b[91m👑♡  𝐬𝐨𝐱Ŧ  👑♡LOGIN EROR ")
      time.sleep(1)
      sys.exit()
  except:
    sys.exit()
...
def login_lagi334():
	try:
		...
		cookie=input(f' 🔜 〘  ༒  〙𓄑>[{h}👑♡  𝐬𝐨𝐱Ŧ  👑♡{x}] Cookies :{asu} ')
		...
		print(f'〘  ༒  〙 𓄑> {x}[{h}°{x}]{h} LOGIN SECCESFULL{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	banner()
	gh = '@S_O_XT'
	print('🔜〘  ༒  〙 𓄑> 𝘾𝙍𝘼𝙆𝙄𝙉𝙂 FILE[1]')
	print('🔜〘  ༒  〙 𓄑> 𝗟𝗢𝗚 𝗨𝗧 [T]')
	____________ = input('🔜〘  ༒  〙 𓄑>𝐂𝐡𝐨𝐬𝐞 :  ')
...
def crack_file():
    try:
        jum = input('🔜〘  ༒  〙 𓄑>𝐂𝐡𝐨𝐬𝐞 𝐍𝐀𝐌𝐄 : ')
        for line in open(jum, 'r').readlines():
            id.append(line.strip())
      #  print(f'➤〘  ༒  〙 𓄑>  :  {h}'+str(len(id)))
        setting()
    except requests.exceptions.ConnectionError:
            exit()
...
def dump_massal():
	...
	for userr in uid:
		try:
			...
			exit()
	try:
		print(f'➤〘  ༒  〙 𓄑>𝗧𝗘𝗦𝗧 𝗜𝗗 𝗣𝗨𝗕𝗟𝗖𝗘➾ {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		back()
	except (KeyError,IOError):
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r⚠︎︎     〖YADA〗׀ {(loop)}׀{len(id)} ׀  ⚠︎︎"),
	sys.stdout.flush()
